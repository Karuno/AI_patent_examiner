...
(function($) {
	$.gd = {
		...
		changeStyle: function(g) {
			...
			h.find('.' + c.switchChooseBtn).each(function() {
				$(this).on('click.changeStyle', function() {
					var a = $(this).attr('name'),
						checked = h.find('.' + c.switchChooseClass).filter('[name=' + a +
							']').filter(':checked'),
			...
			h.find('.' + c.switchChoosedefBtn).on('click.changeStyle', function() {
				var a = c.defaultLinkName,
					...
					defaultInput = h.find('.' + c.switchChooseClass).filter('[value=' + c
						.defaultLinkName + '][name=' + styleGloup + ']');
		...
		rollover: function(f) {
			...
			$(c.area).each(function() {
				var e = $(this).find('img').filter('[src*="' + c.offSuffix + '"]').each(
					function() {
						var a = $(this);
						var b = a.parents('a');
						var d = a.attr('src');
		...
		tab: function(f) {
			...
			$(c.area).find('.' + c.naviClass).each(function(i) {
				var e = $(this).find('a[href*="#"], area[href*="#"]').not(
		...
		switchMenu: function(m) {
			...
			$(c.area).find('.' + c.naviClass).each(function() {
				...
				var j = f.find('.' + c.activeClass).not('.' + c.cntClass).each(
					function() {
						$(this).find('.' + c.cntClass).eq(0).addClass(c.activeClass).show()
					});
				var k = f.find('.' + c.switchClass).css('cursor', 'pointer').on(
					'click.switchMenu',
					function() {
						var a = $(this).parent();
						...
						var b = a.find('.' + c.cntClass).eq(0),
							img = $(this).find('img[src*="' + c.offSuffix + '"], img[src*="' +
								c.onSuffix + '"]'),
							src = img.attr('src');
				...
				var l = $(this).find('li.' + c.activeClass).each(function() {
					var a = $(this).find('img[src*="' + c.offSuffix + '"], img[src*="' +
						c.onSuffix + '"]');
					var b = a.attr('src');
				...
				function changeImg(a, b, d) {
					...
					if (b) {
						...
						a.find('img[src*="' + c.offSuffix + '"], img[src*="' + c.onSuffix +
							'"]').attr('src', b)
					}
				}
			})
